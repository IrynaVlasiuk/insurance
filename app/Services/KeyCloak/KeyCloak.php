...
class KeyCloak {
	...
	public function __construct () {
		 ...
		 * @type {String}
		 */
		$this->realm_id = env('SSO_REALM');

		/**
		 * Client/Application ID
		 * @type {String}
		 */
		$this->client_id = env('SSO_RESOURCE');

		/**
		 * If this is a public application or confidential.
		 * @type {String}
		 */
		$this->is_public = env('SSO_PUBLIC_CLIENT');
		 ...
		 * @type {String}
		 */
		$key_parts = str_split(env('SSO_REALM_PUBLIC_KEY'), 64);
	...
	public function createUser($email, $password){
        $params = array(
            'email' => 'camille.picolet1@avril-conseil.com',
            'username' => 'camille.picolet1@avril-conseil.com'
        );
        ...
        if ($this->is_public) {
            array_push($headers, 'Authorization: Bearer ' . request()->bearerToken());
            array_push($headers, 'Authorization: Basic ' . base64_encode('admin-cli' . ':' . '4630a6c7-1dfd-4f9e-88c0-9dafaf7e9aec'));
        } else {
            array_push($headers, 'Authorization: Basic ' . base64_encode($this->client_id . ':' . $this->secret));
        }

        $response = $this->send_request('POST', '/users', $headers, json_encode($params), true);
        dd($response);
        if ($response['code'] < 200 || $response['code'] > 299) {
            return FALSE;
        } else {
	 ...
	 *
	 */
	public function validate_token ($token, $remote = FALSE) {
		if (!$token) {
			return FALSE;
		}
	...
	**/
	 ...
	 */
	protected function send_request ($method = 'GET', $path = '/', $headers = array(), $data = '', $admin = false) {
		$method = strtoupper($method);
		$url = ($admin ? $this->realm_admin_url : $this->realm_url) . $path;
    ...
    {
        return JWT::decode($token, $this->public_key, ['RS256']);
    }
}