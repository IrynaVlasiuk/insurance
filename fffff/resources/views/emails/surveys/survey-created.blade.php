    ...
    <table role="presentation" border="0" cellpadding="0" cellspacing="0">
        <tr>
            <td>
                ...
                <p>Suite à notre entretien téléphonique,  nous vous confirmons que l expertise pour votre dommage de {{strtolower($survey->claim->damage_type)}} n°{{$survey->claim->external_id}} (contrat {{$survey->claim->contract ? $survey->claim->contract->contract_number : ''}})
                    aura lieu le {{ date('d/m/Y \à H\hi', strtotime($survey->datetime_scheduled)) }}.</p>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <table class="content-table">
                                            <tr>
                                                @if($customer)
                                                <td><b>Client :</b> {{$customer->full_name ? $customer->full_name : ''}}<br>
                                                    {{$customer->address1 ? $customer->address1 : ''}}<br>
                                                    @if(!is_null($customer->address2))
                                                        {{$customer->address2 ? $customer->address2 : ''}}<br>
                                                    @endif
                                                    {{$customer->zipcode ? $customer->zipcode : ''}} {{$customer->city ? $customer->city : ''}}<br>
                                                    <br>
                                            ...
                                            <tr>
                                                ...
                                                <td><b>Agent :</b> {{$agent->full_name}}<br>
                                                    {{$agent->address1 ? $agent->address1 : ''}}<br>
                                                    @if(!is_null($agent->address2))
                                                        {{$agent->address2}}<br>
                                                    @endif
                                                    {{$agent->zipcode ? $agent->zipcode : ''}} {{$agent->city ? $agent->city : ''}}<br>
                                                    <br>
